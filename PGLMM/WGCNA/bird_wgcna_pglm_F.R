...
eigengenes_all_ind <- read.csv(
   "C:/Users/18126/OneDrive - University of Toronto/Projects/bird_expression/wgcna/eigengenes_female.csv")
...
eigenvalues_long <- eigengenes_all_ind %>%
  ...
  mutate(#sex = ifelse(grepl("M", X), "M", "F"), #sex
         ...
         nesting_strat = case_when(grepl("BB", X) ~ "ob_cavity", #Nesting strategy
                                   grepl("BS", X) ~ "open",
                                   grepl("CW", X) ~ "fac_cavity",
                                   grepl("ET", X) ~ "ob_cavity",
                                   grepl("HS", X) ~ "fac_cavity",
...
eigenvalues_long <- as.data.frame(eigenvalues_long[,2:5])

eigenvalues_long$nesting_strat <- factor(eigenvalues_long$nesting_strat, 
                                         levels = c("ob_cavity",
                                                    "fac_cavity",
                                                    "open"
                                         ))
...
eigen_phylo <- function(eigen_df, phylo) {
  ...
  eigen_model <- MCMCglmm(eigenvalue ~ nesting_strat*attrate,
...
write.csv(model_results_df, "wgcna_pglm_female_results.csv")
