...
setwd("C:/Users/18126/OneDrive - University of Toronto/Projects/bird_expression/tan4_plot/")


############################################################################
#   Read in count data
############################################################################
countdata<-as.matrix(read.csv("CN_raw_counts.csv"))
...
metadata <- read.csv(file = "CN_WGCNA_traits.csv", header=T)
...
bwnet6 <- blockwiseModules(normalizedCounts, # This line passes our "normalized_counts" dataset that we created on lines 191-192 to the blockwiseModules function
                           ...
                           power = 6, # I chose a power parameter of 14, based on the plot we generated in the previous step. Do you understand why?
...
metadata$Sex <- relevel(as.factor(metadata$Sex), ref="F")
...
write.csv(mergedMEs6, file="eigengenes_all_ind.csv")
...
save(MEs6, moduleLabels6, moduleColors6, geneTree6, file = "Cavity_Module_Data_All.Rdata")
...
#Convert trait matrix into numeric vectors
...
# labeledHeatmap(Matrix = moduleTraitCor16,
#                xLabels = names(metadata2),
...
write.csv(geneInfo, file="WGCNA_CN_Bothsexes.geneInfo.Pow6.csv")
...
TOM = TOMsimilarityFromExpr(normalizedCounts, power = 6); 
...
inModule=is.finite(match(moduleColors6,modules))
...
cyt = exportNetworkToCytoscape(modTOM,
                               ...
                               nodeAttr = moduleColors6[inModule]);

chooseTopHubInEachModule(normalizedCounts,colorh = "tan4") 
